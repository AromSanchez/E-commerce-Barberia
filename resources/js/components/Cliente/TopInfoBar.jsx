      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="hidden lg:flex justify-between items-center">
          <div className="flex-1 flex items-center space-x-8">
            {/* Mostrar un solo mensaje promocional en desktop */}
            <div className="flex items-center space-x-2 transition-all duration-300">
              <span className="text-gray-400">{promoMessages[currentMessageIndex].icon}</span>
              <span>{promoMessages[currentMessageIndex].text}</span>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-4">
            <span className="text-white text-xs sm:text-sm">Síguenos en:</span>
            <div className="flex space-x-3">
              {socialLinks.map((social, index) => (
                <a
                  ...
                  className="text-gray-400 hover:text-white transform hover:scale-110 transition-all duration-300 sm:w-5 sm:h-5"
                >
                  {social.icon}
                </a>
              ))}
